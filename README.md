...
The slurm files `data_gen.slurm` and `alpha_gen.slurm` run `data_gen.py` and `alpha_gen.py` (respectively) in a HPC shell where SLURM commands are recognized.  Modify the preamble with your credentials.  
